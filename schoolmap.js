var refresh = false; var jQuery, $; jQuery = $ = self._jQuery1; $(document).on('click', '.popupShowWalkingRoute', function (event) {
    popupShowWalkingRoute(event.target.dataset.schoolid);
    //console.info(event.target);
    //console.info(event.target.dataset.schoolid);
});

function popupShowWalkingRoute(schoolId) {
    //console.info("SchoolID: ", schoolId);
    if (!!document.querySelector(".homeDataView").id) {
        $("html, body").animate({
            scrollTop: $('.mapWidget').offset().top
        }, 500);
        // Call path route web service and show polyline on map.

        var mapWidget = dijit.registry.byId(document.querySelector(".mapWidget").id);
        var homeDataView = dijit.registry.byId(document.querySelector(".homeDataView").id);
        var schoolID = schoolId;
        var homeTrackId = homeDataView.mxcontext.trackId;

        if (!(homeTrackId === "")) {
            var easting = homeDataView.mxcontext.trackObject.jsonData.attributes.Easting.value;
            var northing = homeDataView.mxcontext.trackObject.jsonData.attributes.Northing.value;
            ...
            var sr = '<?xml version="1.0" encoding="utf-8"?>' +
                ...
                'xmlns:tns="https://schoolsportal.mendixcloud.com/p/schoolsmap">' +
                '<soap:Body>' +
                '<tns:SchoolGetRoute>' +
                '<FromEasting>' + easting + '</FromEasting>' +
                '<FromNorthing>' + northing + '</FromNorthing>' +
                '<SchoolId>' + schoolID + '</SchoolId>' +
                '</tns:SchoolGetRoute>' +
                '</soap:Body>' +
                '</soap:Envelope>';

            var url = window.location.protocol + "//" + window.location.host + "/ws/WebService_SchoolsRoute"
            ...
            function reqListener() {
                ...
                if (this.status == 200) {
                    parseSchoolGetRouteXML(this);
                }
            }

            function parseSchoolGetRouteXML(xml) {
                var x, y, xmlDoc, parser, xmlCoords, coordsArray;
                xmlDoc = xml.responseXML;
                var pathLatLngs = [];
                x = xmlDoc.getElementsByTagName("Result");

                parser = new DOMParser();
                xmlCoords = parser.parseFromString(x[0].childNodes[0].nodeValue, "text/xml");

                //console.log(x[0].childNodes[0].nodeValue);
                //console.log(xmlCoords);

                y = xmlCoords.getElementsByTagName("gml:coordinates");
                coordsArray = y[0].childNodes[0].nodeValue.split(" ");

                //console.log(coordsArray);

                for (var i = 0; i < coordsArray.length; i++) {
                    var coords = coordsArray[i];
                    // convert to LatLang and add to pathLatLngs.
                    var eastNorth = coords.split(",");
                    var osgb = new GT_OSGB();
                    osgb.setGridCoordinates(eastNorth[0], eastNorth[1]);
                    //console.log("osgb=", osgb);

                    var wgs84 = osgb.getWGS84();
                    var newCoords = [wgs84.latitude, wgs84.longitude];
                    pathLatLngs.push(newCoords);
                }

                //console.log(pathLatLngs);
                var polyline = mapWidget.exposeL.polyline(pathLatLngs, {
                    color: 'green',
                    weight: 5,
                    pane: "travelPane"
                });
                ...
                //Get distance value
                if (homeDataView.mxcontext.trackObject != null) {
                    var schoolMeasureXMLString = homeDataView.mxcontext.trackObject.jsonData.attributes.SchoolMeasureResponse.value;
                    var parser = new DOMParser();
                    var schoolMeasureXML = parser.parseFromString(schoolMeasureXMLString, "text/xml");

                    // var schoolTag = schoolMeasureXML.getElementById(schoolID); // In IE getElementById only works on document
                    var schoolTag = schoolMeasureXML.querySelector("School[id='" + schoolID + "']");

                    var distance = schoolTag.querySelector('DistanceRouted').textContent;
                    // Insert route distance text into <p> tag
                    var textstring = '' + distance + ' ';
                }

                var distance = textstring;
                var tooltipContent = '<i class="fas fa-walking ico-left"></i>' + distance + 'from address';
                polyline.bindTooltip(tooltipContent);
                mapWidget._map.closePopup();
                mapWidget._travelLayerGroup.clearLayers();
                mapWidget._travelLayerGroup.addLayer(polyline);
                polyline.openTooltip();
                mapWidget._map.fitBounds(polyline.getBounds());
            }
        }
    }
}
...
/* widget = snippetjs_widget_snippetjs_0 (snippetjs.js) */